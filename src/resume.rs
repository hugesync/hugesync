...
pub struct ResumeState {
    ...
    /// Local file path (stored as String for bitcode compatibility)
...
impl ResumeManager {
    ...
    fn state_key(local_path: &str, remote_path: &str) -> String {
    ...
    fn state_path(&self, local_path: &str, remote_path: &str) -> PathBuf {
    ...
    pub fn load(&self, local_path: &str, remote_path: &str) -> Result<Option<ResumeState>> {
        ...
        let state: ResumeState = bitcode::decode(&data)
    ...
    fn is_valid(&self, state: &ResumeState) -> Result<bool> {
        // Check if local file exists and matches
        let metadata = match std::fs::metadata(&state.local_path) {
            Ok(m) => m,
    ...
    /// Remove resume state
    pub fn remove(&self, local_path: &str, remote_path: &str) -> Result<()> {
        let path = self.state_path(local_path, remote_path);
        if path.exists() {
            ...
            .map_err(|e| Error::io("reading cache directory", e))?
        {
            ...
            if path.extension().map(|e| e == "bin").unwrap_or(false) {
                if let Ok(data) = std::fs::read(&path) {
                    if let Ok(state) = bitcode::decode::<ResumeState>(&data) {
                        states.push(state);
                    }
                }
            }
